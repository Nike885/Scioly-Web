...
export function EventsProvider({ children }) {
  ...
  console.log('EventsProvider render', { eventsLength: events.length, loading });
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      didTimeout = true;
      setLoading(false);
      console.log('EventsProvider: Timeout reached, setting loading to false');
    }, 5000); // 5 seconds

    loadEvents()
      .catch((err) => {
        console.error('EventsProvider: loadEvents error:', err);
      })
      .finally(() => {
        if (!didTimeout) {
          clearTimeout(timeout);
          setLoading(false);
          console.log('EventsProvider: Finished loading events');
        }
      });
