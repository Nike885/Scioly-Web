...
export default function EventScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    if (eventData) {
      ...
      // Check if current user has already signed up
      if (user && eventData.attendees) {
        const userEmail = user.email || user.sNumber;
        const alreadySignedUp = eventData.attendees.some(attendee => 
          attendee.email === userEmail || attendee.email === user.sNumber
        );
        setHasSignedUp(alreadySignedUp);
        
        // Pre-fill form with user data if they haven't signed up
        if (!alreadySignedUp) {
          setName(user.name || '');
          setEmail(userEmail || '');
        }
  ...
  }, [eventId, getEventById, navigation, user]);
  ...
  const handleSignup = async () => {
    ...
    // Check if capacity reached (default to 50 if not set)
    const eventCapacity = event.capacity || 50;
    if (event.attendees && event.attendees.length >= eventCapacity) {
      setErrorDialog({
        visible: true,
    ...
    try {
      ...
      // Update local state to show user has signed up
      setHasSignedUp(true);
  ...
  const eventCapacity = event.capacity || 50;
  const isFullyBooked = event.attendees && event.attendees.length >= eventCapacity;
          ...
          <View style={styles.eventDetails}>
            ...
            <View style={styles.metadataContainer}>
              ...
              <View style={styles.metadataItem}>
                ...
                <Text style={styles.metadataValue}>
                  {event.attendees ? event.attendees.length : 0} / {eventCapacity} spots filled
                </Text>
              </View>
            </View>
          ...
          {!isAdmin && (
            hasSignedUp ? (
              <View style={styles.alreadySignedUpContainer}>
                ...
                <Text style={styles.alreadySignedUpTitle}>You're Already Signed Up! ✅</Text>
                <Text style={styles.alreadySignedUpText}>
                  You have successfully signed up for this event. We'll see you there!
                </Text>
                <TouchableOpacity 
